...
class Client(ttkb.Frame):
    ...
    def __init__(self, master, serveraddr, serverport, rtpport, filename):
        ...
        self.subscribe_window = None  # 初始化订阅窗口变量
        self.update_info_thread = None  # 初始化更新信息线程变量
        self.createWidgets()
    ...
    def createWidgets(self):
        ...
        self.play_pause_button.pack(side=tk.LEFT, padx=2, pady=2, expand=True)
        
        self.teardown = ttkb.Button(self.buttonFrame, text="Teardown", bootstyle="danger", command=self.exitClient)
        self.teardown.pack(side=tk.LEFT, padx=2, pady=2, expand=True)

        # Create Subscribe button
        self.subscribe = ttkb.Button(self.buttonFrame, text="Subscribe", bootstyle="info", command=self.showSubscribeWindow)
        self.subscribe.pack(side=tk.LEFT, padx=2, pady=2, expand=True)

        self.fullscreen = ttkb.Button(self.buttonFrame, text="Fullscreen", bootstyle="secondary", command=self.toggleFullscreen)
        self.fullscreen.pack(side=tk.LEFT, padx=2, pady=2, expand=True)

        # Add replay button
        self.replay_button = ttkb.Button(self.buttonFrame, text="Replay", bootstyle="warning", command=self.replayMovie)
        self.replay_button.pack(side=tk.LEFT, padx=2, pady=2, expand=True)
        ...
        self.icon_label = tk.Label(self.master, bg='black')
    ...
    def showSubscribeWindow(self):
        """Create a new window to display subscription information。"""
        if self.subscribe_window is not None and self.subscribe_window.winfo_exists():
            self.subscribe_window.destroy()
            self.subscribe_window = None
        else:
            self.subscribe_window = tk.Toplevel(self.master)
            self.subscribe_window.title("Subscription Information")
            self.subscribe_window.geometry("400x200")

            self.table_frame = ttk.Frame(self.subscribe_window)
            self.table_frame.pack(fill=tk.BOTH, expand=True)

            self.table_label = ttk.Label(self.table_frame, text="Information Table", font=("Arial", 16))
            self.table_label.pack()

            if not self.update_info_thread or not self.update_info_thread.is_alive():
                self.update_info_thread = threading.Thread(target=self.updateInfoPeriodically)
                self.update_info_thread.daemon = True
                self.update_info_thread.start()

    def updateInfoPeriodically(self):
        """Periodically update the information table。"""
        while not self.exit_event.is_set():
            try:
                if self.state == self.PLAYING and self.table_frame.winfo_viewable():
                    self.updateInfoTable()
            except tk.TclError:  # 捕获窗口已经被销毁的异常
                break
            time.sleep(1)  # 每秒更新一次
    ...
    def updateInfoTable(self):
        ...
        data_rate = (self.total_bytes_received * 8) / elapsed_time  # bits per second
        packet_loss_rate = ((self.frameNbr - self.total_packets_received) / self.frameNbr) * 100 if self.frameNbr > 0 else 0

        data = [
            ("Name", "Value"),
            ("Frame", self.frameNbr),
            ("Status", "Subscribed"),
            ("Total Packets Received", self.total_packets_received),
            ("Total Bytes Received", self.total_bytes_received),
            ("Data Rate", f"{data_rate:.2f} bits per second"),
            ("Packet Loss Rate", f"{packet_loss_rate:.8f}%")
        ]

        for widget in self.table_frame.winfo_children():
            widget.destroy()
        for i, (name, value) in enumerate(data):
            ttk.Label(self.table_frame, text=name).grid(row=i, column=0, sticky='w')
            ttk.Label(self.table_frame, text=value).grid(row=i, column=1, sticky='w')

    def exitClient(self):
        """Teardown button handler。"""
        if self.request:
            self.sendRtspRequest(self.TEARDOWN)
            self.recvRtspReply()  # Ensure to receive the TEARDOWN reply and process it
        else:
            self.request = False
        self.exit_event.set()  # 设置事件来终止子线程
    ...
    def replayMovie(self):
        ...
        if self.state in [self.READY, self.PLAYING]:
            self.sendRtspRequest(self.TEARDOWN)  # First, teardown the current session
            self.recvRtspReply()  # Ensure to receive the TEARDOWN reply and process it
            self.resetState()  # Reset state
            self.master.destroy()  # Close the current window

            # Create a new window and new client instance for replay
            root = tk.Tk()
            new_client = Client(root, self.serverAddr, self.serverPort, self.rtpPort, self.fileName)
            new_client.master.title("RTPClient - Replay")
            root.mainloop()
    ...
    def resetState(self):
        ...
        self.teardownAcked = 0
        self.requestSent = -1
        self.exit_event.set()  # 确保所有线程可以安全退出
    ...
    def connectToServer(self):
        ...
        self.rtspSocket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # Enable address reuse
        try:
    ...
    def openRtpPort(self):
        ...
        self.rtpSocket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # Enable address reuse
        self.rtpSocket.settimeout(0.5)
        try:
            self.rtpSocket.bind(('0.0.0.0', self.rtpPort))
            print(f"Successfully bound to port {self.rtpPort}")
        except Exception as e:
            showwarning('Unable to Bind', f'Unable to bind PORT={self.rtpPort}')
            print(f"Error binding to port {self.rtpPort}: {e}")
        
    def handler(self):
        """Handler on explicitly closing the GUI window。"""
        self.pauseMovie()
        if showwarning("Quit?", "Are you sure you want to quit?"):
            self.exitClient()
