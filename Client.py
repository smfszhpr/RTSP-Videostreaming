...
class Client(ttkb.Frame):
	...
	def createWidgets(self):
		...
		# 快进按钮
		self.fast_forward_button = ttkb.Button(self.buttonFrame, text=">>", bootstyle="warning", command=self.fast_forward)
		self.fast_forward_button.pack(side=LEFT, padx=2, pady=2, expand=True)

		# 回退按钮
		self.rewind_button = ttkb.Button(self.buttonFrame, text="<<", bootstyle="warning", command=self.rewind)
	...
	def sendRtspRequest(self, requestCode):
		...
		# Teardown request
			...
			request = "FAST_FORWARD" + self.fileName + "RTSP/1.0\nCSeq: " + str(self.rtspSeq) + "\nSession: " + str(self.sessionId)
		...
		if requestCode in [self.SETUP, self.PLAY, self.PAUSE, self.TEARDOWN, self.FAST_FORWARD, self.REWIND]:
		...
		if seqNum == self.rtspSeq:
			...
			if self.sessionId == session:
				if int(lines[0].split(' ')[1]) == 200:
					...
					elif self.requestSent == self.FAST_FORWARD:
						#self.state = self.PLAYING
						print("State updated to PLAYING")
